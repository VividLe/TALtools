...
def args_parser():
    ...
    parser.add_argument('-name', default='cam')
    args = parser.parse_args()
    return args


def post_process(cfg, actions_json_file, writer, best_mAP, info, epoch, name):
    mAP, average_mAP = evaluate_mAP(cfg, actions_json_file, os.path.join(cfg.BASIC.CKPT_DIR, cfg.DATASET.GT_FILE), cfg.BASIC.VERBOSE)

...
from utils.utils import decay_lr, save_best_model
...
def main():
    ...
    for epoch in range(1, cfg.TRAIN.EPOCH_NUM+1):
        ...
        if cfg.BASIC.VERBOSE:
            print('loss: cas %f, cam %f, consistency %f, norm %f, cam_inv %f' % (
            loss_average_cas, loss_average_cam, loss_average_consistency, loss_average_norm, loss_average_cam_inv))
        ...
        if epoch % cfg.TEST.EVAL_INTERVAL == 0:
            ...
            save_best_model(cfg, epoch=epoch, model=model, optimizer=optimizer, name='cas')
    writer.close()
